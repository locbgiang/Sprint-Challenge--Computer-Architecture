    ...
    def run(self):
        ...
        while running:
            ...
            if inst == 130:                 # this is LDI, saving the number to reg
                ...
                self.pc += 3
            elif inst == 167:                 # CMP, compare two value
                first_num = self.ram[self.pc + 1]
                second_num = self.ram[self.pc + 2]
                if self.reg[first_num] == self.reg[second_num]:
                    ...
                    self.G = 1
                self.pc += 3
            elif inst == 84:                  # JMP, jump to the address stored in register
                jumping = self.ram[self.pc + 1]
                self.pc = self.reg[jumping]
            elif inst == 85:                  # JEQ, when equal flag is = 1, jump to given address
                if self.E == 1:
                    jumping = self.ram[self.pc + 1]
                    self.pc = self.reg[jumping]
                else:
                    self.pc += 2
            elif inst == 86:                  # JNE, when equal flag is = 0, jump to given address
                if self.E == 0:
                    jumping = self.ram[self.pc + 1]
                    self.pc = self.reg[jumping]
                else:
